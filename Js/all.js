    ...
    if (isInsider) {
        ...
        if (paramIndex !== -1) {
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                if (paramPair[0] === "insider") {
                    let deskNumber=paramPair[1];
                    break;
                }
            }
        }
        login(paramPair[1]+'@store.com', 'abc123');
        return;
    }
    //檢查登入狀態
...
function demoInput(demoName) {
    switch (demoName) {
        case '阿姨':
            $('#loginEmail').val('jahouba@gamil.com');
...
function postCartOrder(order) {
    ...
    axios.post(`${urlDomain}/600/orders`, order, {
        ...
        console.log(response)
        gaPurchase(order);
        sweetSuccess('請至櫃檯結帳', '訂單送出成功', 2500);
        switchModal();
...
function renderNavList() {
    ...
    let userNameContent = "";
    let loginoutContent = `<span class="nav-link finger" href="" onclick="showLoginModal('login')">店家登入</span>`;
    if (isLogin) {
        let helloStr = getDataFromLocalStorage('_user').role == 'insider' ? '桌號 ' : '登入者:';
        userNameContent = `
...
function renderQrCode() {
    ...
    let str = "http://127.0.0.1:5500/onlineOrder-main/index.html?insider=A3"
